...
This is the final exam for student number: **63027346**.  Confirm your student number is correct before proceeding.  **Do not complete someone else's exam.** 
...
* For these questions, download the answer sheet for your exam, which can be found [here](https://raw.githubusercontent.com/GEOS270/FinalExam/main/sheets/63027346.zip).  
...
For these questions, **make sure** to use the answer sheet, which can be found which can be found [here](https://raw.githubusercontent.com/GEOS270/FinalExam/main/sheets/0.zip).  
...
**4**	
The [  <sub>4.0</sub>  ] arises when we aggregate individual data values into larger [  <sub>4.1</sub>  ] with arbitrary boundaries.

**5**	
An attribute [  <sub>5.0</sub>  ] contains non-spatial data, that is linked to spatial data using an [  <sub>5.1</sub>  ].

**6**	
Spatial [  <sub>6.0</sub>  ] is the process of estimating the values between point observations to create a continuous surface. A common method known as [  <sub>6.1</sub>  ] uses a function to estimate cell values based on their proximity to a set of known point observations.

**7**	
Random sampling is a/an [  <sub>7.0</sub>  ] method for collecting spatial information.  Stratified random sampling and cluster sampling are [  <sub>7.1</sub>  ] methods for collecting spatial information that are designed to help address some of the downsides of random sampling.

**8**	
Resolution is a branch of your [  <sub>8.0</sub>  ], scale is a property of your [  <sub>8.1</sub>  ].
